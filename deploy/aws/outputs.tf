...
output "deployment_summary" {
  ...
  value = <<-EOT
    ...
    Aurora PostgreSQL:
      - Cluster ID: ${aws_rds_cluster.bbgo.cluster_identifier}
      - Endpoint: ${aws_rds_cluster.bbgo.endpoint}
      - Database: ${aws_rds_cluster.bbgo.database_name}
    ...
    Next Steps:
      ...
      3. Check BBGO service: sudo systemctl status bbgo
      4. View logs: tail -f ~/bbgo-prod/bbgo.log
      5. Test database: psql -h ${aws_rds_cluster.bbgo.endpoint} -U ${var.db_username} -d ${var.db_name}
